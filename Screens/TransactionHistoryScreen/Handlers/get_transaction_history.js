...
const getTransactionHistory = async (account) => {
    ...
    try {
        ...
        for (let i = 0; i < history.result.transactions.length; i++) {
            ...
            if (history.result.transactions[i].meta.TransactionResult !== 'tesSUCCESS') {
            ...
            if (history.result.transactions[i].tx.Amount) {
                if (typeof history.result.transactions[i].tx.Amount === "object") {
                    transaction.amount = Number(history.result.transactions[i].tx.Amount.value).toLocaleString("en-US");
                    transaction.currency = xrpl.convertHexToString(history.result.transactions[i].tx.Amount.currency);
                } else {
                    transaction.amount = parseInt(Number(history.result.transactions[i].tx.Amount) * Math.pow(10, -6)).toLocaleString("en-US"); //
                }
            } else if (history.result.transactions[i].tx.LimitAmount) {
                if (typeof history.result.transactions[i].tx.LimitAmount === "object") {
                    transaction.amount = Number(history.result.transactions[i].tx.LimitAmount.value).toLocaleString("en-US");
                    transaction.currency = xrpl.convertHexToString(history.result.transactions[i].tx.LimitAmount.currency);
                } else {
                    transaction.amount = parseInt(Number(history.result.transactions[i].tx.LimitAmount) * Math.pow(10, -6)).toLocaleString("en-US"); //
                }
            }

            const formattedDate = moment.unix(Number(history.result.transactions[i].tx.date) + 946684800).format('MMDDYYYY');
