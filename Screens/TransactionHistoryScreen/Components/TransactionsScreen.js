        ...
        <TouchableOpacity onPress={toggleHeight}>
            <View style={[styles.transactionWrapper, {height: height}]}>
                <View style={styles.transactionTop}>
                    ...
                    <Text style={styles.date}>{props.month}<Text style={styles.unsupportedCharacter}>/</Text>{props.day}<Text style={styles.unsupportedCharacter}>/</Text>{props.year}</Text>
                </View>
                <View style={styles.transactionBottom}>
                    ...
                    {(props.transactionType === "Payment Sent" &&
                        <Text style={styles.transactionAmountPaymentSent}>-{props.transactionAmount} {props.currency}</Text>)
                    || (props.transactionType === "Payment Received" &&
                        <Text style={styles.transactionAmountPaymentReceived}>+{props.transactionAmount} {props.currency}</Text>)
                    || (props.transactionType === "Payment Failed" &&
                        <Text style={styles.transactionAmountPaymentFailed}>{props.transactionAmount} {props.currency}</Text>)
                    ||
                        <Text style={styles.transactionAmount}>{props.transactionAmount} {props.currency}</Text>
                    }
                </View>
                ...
                <View style={styles.transactionBottom}>
                    ...
                    <TouchableOpacity style={styles.viewButton} onPress={() => {
                        Linking.openURL(`https://livenet.xrpl.org/transactions/${props.hash}`);
                    }}>
                        <View style={styles.buttonWrapper}>
                            <Text style={styles.buttonText}>View on Ledger</Text>
                            <MaterialCommunityIcons name={"cube-scan"} size={20} color={'#222222'} />
                        </View>
                    </TouchableOpacity>
                </View>
                }
...
const TransactionsScreen = ({navigation}) => {
    ...
    React.useEffect(() => {
        const getHistory = async () => {
            const history = getTransactionHistory(activeAccount);
            return history;
        }
        setLoading(true);
        getHistory()
        ...
        .catch(error => {
            setLoading(false);
            setErrorMessage("Error: Cannot Fetch Account History for Inactive Account.");
        })
    }, []);
        ...
        <SafeAreaView style={{ backgroundColor: colors.bg }}>
            ...
            <View style={styles.bg}>
                ...
                <ScrollView style={styles.transactionsWrapper}>
                    {!loading && errorMessage.length === 0 && txHistory.map((tx, index) => (
                        <Transaction key={index} setCopiedModalOpen={setCopiedModalOpen} accountInvolved={tx.accountInvolved} transactionType={tx.transactionType} transactionAmount={tx.amount} currency={tx.currency} month={tx.month} day={tx.day} year={tx.year} hash={tx.hash} />
                    ))}
                    {loading && <Text style={styles.loadingText}>Loading...</Text>}
                    {errorMessage.length > 0 && <Text style={styles.loadingText}>{errorMessage}</Text>}
                </ScrollView>
