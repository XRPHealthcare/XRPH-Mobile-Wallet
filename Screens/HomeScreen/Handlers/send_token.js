...
const send_token = async ({ seed, to, currency, amount, memo, destinationTag, balances }) => {
    try {
        const issuerAddress = "rM8hNqA3jRJ5Zgp3Xf3xzdZcx2G37guiZk";
        const currencyCode = currency === "XRP" ? currency :  xrpl.convertStringToHex(currency).padEnd(40, 0);
        ...
        console.log(`Sending ${amount} ${currencyCode} from ${accountA.classicAddress} to ${to}...`)
        const pay_result_A_to_B = await client.submitAndWait(pay_signed_A_to_B.tx_blob)
        if (pay_result_A_to_B.result.meta.TransactionResult == "tesSUCCESS") {
        console.log(`Transaction succeeded: https://livenet.xrpl.org/transactions/${pay_signed_A_to_B.hash}`)
        } else {
        ...
        for (let i = 0; i < balances.length; i++) {
            ...
            if (balances[i].currency === "XRP") {
                ...
                fromBalances[i].value = String(Number(fromBalances[i].value) - xrpl.dropsToXrp(Number(pay_prepared_A_to_B.Fee)));
                if (balances[i].currency === currency) {
                    fromBalances[i].value = String(Number(fromBalances[i].value) - (Number(amount)));
                }
            } else {
                if (balances[i].currency === currency) {
                    fromBalances[i].value = String(Number(fromBalances[i].value) - Number(amount));
        ...
        return {
            error: "Error: Trouble Connecting To The Rippled Server. Please check your internet connection."
        };
    }
}
